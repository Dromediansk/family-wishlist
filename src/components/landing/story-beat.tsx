 ...
 *
 * A `full` beat is the exception: `MockSplit` already spends its own width on
 * two side-by-side viewpoint cards, so nesting it in this beat's other
...
export async function StoryBeat({ beat }: Readonly<{ beat: BeatSpec }>) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          // `side` only exists on a `beside` beat — see BeatSpec — so a
          // `full` beat has nothing to order against here.
          beat.layout === "beside" && beat.side === "start" && "sm:order-last",
        )}
      >
